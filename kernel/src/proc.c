...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  np->state = RUNNABLE;
  np->uid = np->parent->uid; //Each child process inherits its parent's uid
 ...
 */
int setuid(int uid) {
  if(uid < 0x0 || uid > 0xFFFF) { //Valid ranges are 0x0-0xFFFF
    return -1;
  } else if(myproc()->uid != 0) { //Only procs with uid 0 may change their uid through setuid
    return -1;
  } else {
    //acquire(&ptable.lock);
    myproc()->uid = uid;
...
int chown(const char *filename, int uid) {
  begin_op(); // Have to call begin_op here, because namex requires begin_op to have been called before it is called
  char *path = (char *)filename;
  struct proc* p = myproc();
  struct inode* ip = namei(path);
  ilock(ip);
  if(uid < 0x0 || uid > 0xFFFF) { // Valid ranges are 0x0-0xFFFF
    //cprintf("UID not valid\n");
    iunlockput(ip);
    end_op();
    return -1;
  } else if (p->uid != 0 && p->uid != ip->owner) {
    //cprintf("Current proc is not owner\n");
    iunlockput(ip);
    end_op();
    return -1;
  } else {
...
int chmod(const char *filename, int perm) {
  ...
  struct inode* ip = namei(path);
  ilock(ip);
  if(perm < 0x0 || perm > PROT_RW) { // Valid ranges are 0b00-0b11
    iunlockput(ip);
    end_op();
    return -1;
  } else if (p->uid != 0 && p->uid != ip->owner) {
...
int getown(const char *filename) {
   char *path = (char *)filename;
    struct inode* ip = namei(path);
    int ret = 0;
    ilock(ip);
...
int getmod(const char *filename) {
    char *path = (char *)filename;
    struct inode* ip = namei(path);
    int ret = 0;
    ilock(ip);
